...
class SaleTransactionPage extends BasePage {
  ...
  async verifyTransactionErrorMessages(test, expect, expectedErrors) {
    ...
    if (expectedErrors.buyerName)
      expect
        .soft(actualErrors.buyerName, 'Buyer Name Error should match')
        .toBe(expectedErrors.buyerName);
    if (expectedErrors.buyerEmail)
      expect
        .soft(actualErrors.buyerEmail, 'Buyer Email Error should match')
        .toBe(expectedErrors.buyerEmail);
    if (expectedErrors.buyerPhone)
      expect
        .soft(actualErrors.buyerPhone, 'Buyer Phone Error should match')
        .toBe(expectedErrors.buyerPhone);
    if (expectedErrors.propertyAddress)
      expect
        .soft(actualErrors.propertyAddress, 'Property Address Error should match')
        .toBe(expectedErrors.propertyAddress);
    if (expectedErrors.postalCode)
      expect
        .soft(actualErrors.postalCode, 'Postal Code Error should match')
        .toBe(expectedErrors.postalCode);
    if (expectedErrors.salePrice)
      expect
        .soft(actualErrors.salePrice, 'Sale Price Error should match')
        .toBe(expectedErrors.salePrice);
    // Map 'fileName' from test data to 'ownershipDocument' in page object
    if (expectedErrors.fileName)
      expect
        .soft(actualErrors.ownershipDocument, 'Ownership Document Error should match')
        .toBe(expectedErrors.fileName);
