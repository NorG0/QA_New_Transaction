...
class TransactionSummaryPage extends BasePage {
    ...
    constructor(page) {
        ...
        this.transactionSummaryTitle = page.getByRole('heading', { name: 'Transaction Summary', level: 1 });
    ...
    extractFieldsValue(fieldText) {
        ...
        const [transactionID, buyerName, buyerEmail, buyerPhone,
            propertyAddress, postalCode, salePrice,
            ownerShipDocName] = values;

        return {
            transactionID, buyerName, buyerEmail, buyerPhone,
            propertyAddress, postalCode, salePrice,
            ownerShipDocName
        }
    }
    ...
    async verifyTransactionData(test, expect, testData) {
        ...
        expect.soft(actualData.propertyAddress, 'Property Address should match').toBe(testData.propertyAddress);
        ...
        expect.soft(actualData.transactionID.length, 'Transaction ID should not be empty').toBeGreaterThan(0);
...
module.exports = { TransactionSummaryPage };
